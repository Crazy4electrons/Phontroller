...
(function() { // Immediately Invoked Function Expression to create a private scope
    ...
    const OBSTACLE_DISTANCE_THRESHOLD_CM = 25;
     ...
     * @param commandPress The command string to send on 'pointerdown'.
     * @param commandRelease The command string to send on 'pointerup' or 'pointerleave' (optional).
    ...
    function updateUI(data) {
        ...
        if ((data.BackObstacleDetected !== undefined || data.FrontObstacleDetected !== undefined) && DOMElements.obstacleDetected) {
            const backObstacle = data.BackObstacleDetected === true || data.BackObstacleDetected === 1;
            const frontDist = parseFloat(data.FrontObstacleDetected);
            ...
            const obstaclePresent = backObstacle || frontObstacle;
    ...
    function setupEventListeners() {
        ...
        if (DOMElements.steeringWheelContainer && DOMElements.steeringWheel) {
            ...
            let center = { x: 0, y: 0 }; // Stores the calculated center point of the steering wheel container.
  
            // @brief Recalculates the center coordinates of the steering wheel container.
            function updateCenter() {
                ...
                // console.log("Steering wheel center updated:", center); // Debugging line commented out
            }
  
            // Update center coordinates on window resize and after initial layout.
            window.addEventListener('resize', updateCenter);
            setTimeout(updateCenter, 100); // Initial calculation after a short delay to allow layout to settle.
  
            // Handle the start of a drag gesture on the steering wheel.
            sw.addEventListener('pointerdown', (e) => {
                e.preventDefault();
                isSteeringDragging = true; // Set dragging flag.
                sw.setPointerCapture(e.pointerId); // Capture pointer events to ensure smooth dragging even if pointer leaves the element.
                updateCenter(); // Recalculate center just in case the layout changed.
                 // console.log("Steering wheel drag start."); // Debugging line commented out
            });
            ...
            sw.addEventListener('pointermove', (e) => {
                ...
                // Calculate the angle between the pointer position and the center of the container.
                const deltaX = e.clientX - center.x;
                const deltaY = e.clientY - center.y;
                // atan2(-deltaX, deltaY) calculates the angle relative to the upward vertical axis (negative Y).
                const angleFromUpRad = Math.atan2(-deltaX, deltaY);
  
                // Convert the angle from radians to degrees.
                let angleFromUpDeg = angleFromUpRad * (180 / Math.PI);
  
                // Constrain the visual rotation of the steering wheel UI to the defined limit.
                const uiAngleDeg = Math.max(-STEERING_LIMIT_DEG, Math.min(STEERING_LIMIT_DEG, angleFromUpDeg));
  
                // Apply the constrained angle to the CSS variable to visually rotate the wheel.
                sw.style.setProperty('--rotate-angle', `${uiAngleDeg}deg`);
  
                // Map the UI angle (-STEERING_LIMIT_DEG to +STEERING_LIMIT_DEG) to the servo angle (0 to 180).
                // This maps the UI left position to servo 0, center to servo 90, and right to servo 180.
                const servoAngle = Math.round(
                    (uiAngleDeg + STEERING_LIMIT_DEG) / (2 * STEERING_LIMIT_DEG) * 180
                );
  
                // Send the steering command (e.g., "F90") using the throttled function.
                sendWebSocketDataThrottled(`${Commands.STEER_PREFIX}${servoAngle}`);
                 // console.log(`UI Angle: ${uiAngleDeg.toFixed(1)}deg, Servo Angle: ${servoAngle}`); // Debugging line commented out
            });
  
            // @brief Handles the end of a drag gesture (pointer up or cancel).
            const dragEnd = (e) => {
                if (!isSteeringDragging) return;
                isSteeringDragging = false; // Clear dragging flag.
                sw.releasePointerCapture(e.pointerId); // Release pointer capture.
  
                // Reset the steering wheel UI appearance to centered.
                sw.style.setProperty('--rotate-angle', '0deg');
                // lastSteeringAngleRad = 0; // Variable commented out
  
                // Send a center steering command to the ESP8266.
                sendWebSocketData(Commands.CENTER);
                 // console.log("Steering wheel drag end."); // Debugging line commented out
            };
  
            // Add event listeners for the end of the drag.
            sw.addEventListener('pointerup', dragEnd);
            sw.addEventListener('pointercancel', dragEnd); // Handle interruptions like pressing Alt+Tab.
        }
        ...
        // DOMElements.body.addEventListener('pointerdown', (e) => { e.preventDefault(); }); // Commented out, uncomment if needed.
    }
